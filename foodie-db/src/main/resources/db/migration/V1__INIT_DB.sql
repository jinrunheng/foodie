...
CREATE DATABASE IF NOT EXISTS foodie_db CHARACTER SET utf8 COLLATE utf8_general_ci;;

-- 创建轮播图数据表
DROP TABLE IF EXISTS `foodie_db`.`carousel`;
CREATE TABLE `foodie_db`.`carousel`
(
...
-- 创建商品分类数据表
DROP TABLE IF EXISTS `foodie_db`.`category`;
CREATE TABLE `foodie_db`.`category`
(
...
3:三级小分类',
    ...
    cat_image VARCHAR(128) COMMENT '分类图',
...
-- 创建用户地址数据表
DROP TABLE IF EXISTS `foodie_db`.`user_address`;
CREATE TABLE `foodie_db`.`user_address`
(
...
-- 创建订单状态表
DROP TABLE IF EXISTS `foodie_db`.`order_status`;
CREATE TABLE `foodie_db`.`order_status`
(
